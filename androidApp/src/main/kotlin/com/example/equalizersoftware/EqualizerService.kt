...
class EqualizerService : Service() {
    ...
    fun setBandLevel(band: Short, level: Short) {
        // Map level from 0-15 to dB: 0 = -15dB, 7.5 = 0dB, 15 = +15dB
        // But we want to BOOST only, so: 0 = 0dB, 15 = +15dB
        val db = level.toFloat()
    ...
    fun setEqualizerEnabled(enabled: Boolean) {
        try {
            ...
            Log.d("EqualizerService", "Equalizer enabled: $enabled")
        } catch (e: Exception) {
            Log.e("EqualizerService", "setEqualizerEnabled failed", e)
        }
    }
